...
public class Ques726
{
    ...
    private static void closedTour(int one, int two)
    {
        if (currentArr.size() != 0)
        {
            for (int count = 0; count < currentArr.size(); count++)
            {
                if (currentRow == (one + vertical[currentArr.get(count)]))
                {
                    if (currentColumn == (two + horizontal[currentArr.get(count)]))
